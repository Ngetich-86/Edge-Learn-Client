...
const EmailSender = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    setResponse(null); // Reset response on form submit

    try {
      const res = await axios.post(`${APIDomain}/send-email`, formData);
      setResponse(res.data.message || "Email sent successfully"); // Save response from server
      toast.success("Email sent successfully");
    } catch (error: any) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-base">
      ...
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        ...
        {/* Response Message */}
        {response && (
          <div className="mt-4 p-4 text-center text-white rounded-md bg-blue-500">
            {response}
          </div>
        )}
      </div>
